...
- **功能可开关** —— 设置里像 Obsidian 一样拨插件：文章分享、同步日志、AI 接口、MCP，关掉立刻从界面消失、已发出的链接立刻失效，数据还在，重开就回来
...
| `OWIKI_SHARE` / `OWIKI_SYNCLOG` / `OWIKI_APIKEYS` / `OWIKI_MCP` | （空=开） | 对应功能的**首次启动**默认；设为 `off` 则默认关闭。之后以 Web「设置 → 插件」为准，拨过开关环境变量不再覆盖 |
